...
public class FinanzasPanel extends JPanel {
	...
	private static final String[] COLORES_PRESET = { "#FF6B6B", "#FF9F43", "#FECA57", "#48DBFB", "#1DD1A1", "#A29BFE",
	...
	private enum Periodo {
		SEMANA, MES, TODO, TARJETA
	}

	private Periodo periodoActual = Periodo.MES;
	private Tarjeta tarjetaFiltro = null;
	...
	private JPanel buildSidebar() {
		...
		cbPeriodo.addActionListener(e -> {
			int idx = cbPeriodo.getSelectedIndex();
			if (idx == 0) {
				periodoActual = Periodo.MES;
				tarjetaFiltro = null;
			} else if (idx == 1) {
				periodoActual = Periodo.SEMANA;
				tarjetaFiltro = null;
			} else if (idx == 2) {
				periodoActual = Periodo.TODO;
				tarjetaFiltro = null;
			} else {
				int tIdx = idx - 3;
				if (tIdx >= 0 && tIdx < tarjetas.size()) {
					periodoActual = Periodo.TARJETA;
					tarjetaFiltro = tarjetas.get(tIdx);
				}
			}
			actualizarTodo();
		});
		...
		// ── Sección tarjetas ──────────────────────────────────
		JPanel secTarjetas = new JPanel(new BorderLayout());
		secTarjetas.setBackground(BG_SIDEBAR);
		secTarjetas.setName("secTarjetas");

		JPanel tarjHeader = new JPanel(new BorderLayout());
		tarjHeader.setBackground(BG_SIDEBAR);
		tarjHeader.setBorder(new CompoundBorder(new MatteBorder(1, 0, 0, 0, BORDER_LIGHT),
				BorderFactory.createEmptyBorder(7, 12, 5, 10)));

		JLabel lblTarj = new JLabel("Tarjetas");
		lblTarj.setFont(new Font("SansSerif", Font.BOLD, 12));
		lblTarj.setForeground(TEXT_MAIN);

		JButton btnAddTarj = new JButton("+");
		btnAddTarj.setFont(new Font("SansSerif", Font.BOLD, 16));
		btnAddTarj.setForeground(ACCENT);
		btnAddTarj.setContentAreaFilled(false);
		btnAddTarj.setBorderPainted(false);
		btnAddTarj.setFocusPainted(false);
		btnAddTarj.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
		btnAddTarj.setToolTipText("Agregar tarjeta");
		btnAddTarj.addActionListener(e -> dlgTarjeta(null));

		tarjHeader.add(lblTarj, BorderLayout.WEST);
		tarjHeader.add(btnAddTarj, BorderLayout.EAST);

		JPanel listaTarj = new JPanel();
		listaTarj.setLayout(new BoxLayout(listaTarj, BoxLayout.Y_AXIS));
		listaTarj.setBackground(BG_SIDEBAR);
		listaTarj.setName("listaTarjetas");

		secTarjetas.add(tarjHeader, BorderLayout.NORTH);
		secTarjetas.add(listaTarj, BorderLayout.CENTER);
		...
		JPanel todasSecciones = new JPanel();
		todasSecciones.setLayout(new BoxLayout(todasSecciones, BoxLayout.Y_AXIS));
		todasSecciones.setBackground(BG_SIDEBAR);
		todasSecciones.add(secTarjetas);
		todasSecciones.add(sidebarCats);

		JScrollPane scroll = new JScrollPane(todasSecciones);
		...
		totalPanel.setBorder(new CompoundBorder(new MatteBorder(1, 0, 0, 0, BORDER_LIGHT),
	...
	/** Actualiza las tarjetas visibles en el sidebar */
	private void actualizarSidebarTarjetas() {
		// Buscar el panel listaTarjetas por nombre
		Component[] comps = ((JPanel) ((JScrollPane) ((JPanel) ((JSplitPane) getComponent(0)).getLeftComponent())
				.getComponent(1)).getViewport().getView()).getComponents();

		JPanel listaTarj = null;
		for (Component c : comps) {
			if (c instanceof JPanel p && "secTarjetas".equals(p.getName())) {
				for (Component cc : p.getComponents()) {
					if (cc instanceof JPanel pp && "listaTarjetas".equals(pp.getName())) {
						listaTarj = pp;
						break;
					}
				}
		...
		for (Tarjeta t : tarjetas) {
			listaTarj.add(buildTarjetaCard(t));
		}
		listaTarj.revalidate();
		listaTarj.repaint();
	}

	private JPanel buildTarjetaCard(Tarjeta t) {
		long diasCorte = t.getDiasParaCorte();
		long diasPago = t.getDiasParaPago();
		Color tColor = parseColor(t.getColor());

		JPanel card = new JPanel(new BorderLayout(0, 2));
		card.setBackground(BG_SIDEBAR);
		card.setBorder(
				new CompoundBorder(new MatteBorder(0, 3, 0, 0, tColor), BorderFactory.createEmptyBorder(6, 10, 6, 10)));
		card.setMaximumSize(new Dimension(Integer.MAX_VALUE, 80));
		card.setAlignmentX(Component.LEFT_ALIGNMENT);

		JLabel lblNombre = new JLabel("💳  " + t.getNombre());
		lblNombre.setFont(new Font("SansSerif", Font.BOLD, 12));
		lblNombre.setForeground(tColor);

		JLabel lblPeriodo = new JLabel(t.getPeriodoLabel());
		lblPeriodo.setFont(new Font("SansSerif", Font.PLAIN, 10));
		lblPeriodo.setForeground(TEXT_DIM);

		String corteStr = diasCorte == 0 ? "¡Hoy es corte!"
				: diasCorte > 0 ? "Corte en " + diasCorte + "d" : "Corte hace " + Math.abs(diasCorte) + "d";
		String pagoStr = diasPago == 0 ? "¡Hoy vence pago!"
				: diasPago > 0 ? "Pago en " + diasPago + "d" : "⚠ Pago vencido " + Math.abs(diasPago) + "d";

		Color pagoColor = diasPago <= 3 ? COLOR_OVER : diasPago <= 7 ? COLOR_WARN : TEXT_DIM;

		JPanel info = new JPanel(new GridLayout(2, 1, 0, 1));
		info.setBackground(BG_SIDEBAR);
		JLabel lblCorte = new JLabel(corteStr);
		lblCorte.setFont(new Font("SansSerif", Font.PLAIN, 10));
		lblCorte.setForeground(diasCorte <= 3 ? COLOR_WARN : TEXT_DIM);
		JLabel lblPago = new JLabel(pagoStr);
		lblPago.setFont(new Font("SansSerif", Font.PLAIN, 10));
		lblPago.setForeground(pagoColor);
		info.add(lblCorte);
		info.add(lblPago);

		card.add(lblNombre, BorderLayout.NORTH);
		card.add(lblPeriodo, BorderLayout.CENTER);
		...
		MouseAdapter ctx = new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				if (SwingUtilities.isRightMouseButton(e)) {
					...
					del.setForeground(COLOR_OVER);
					edit.addActionListener(ev -> dlgTarjeta(t));
					del.addActionListener(ev -> {
						if (JOptionPane.showConfirmDialog(FinanzasPanel.this,
								"¿Eliminar tarjeta \"" + t.getNombre() + "\"?", "Confirmar",
		...
		card.addMouseListener(ctx);
		for (Component c : card.getComponents())
			if (c instanceof JComponent jc)
				jc.addMouseListener(ctx);

		return card;
	}

	private void dlgTarjeta(Tarjeta tarjetaExistente) {
		boolean nueva = tarjetaExistente == null;
		JTextField tfNombre = dlgField(nueva ? "" : tarjetaExistente.getNombre());
		JTextField tfCorte = dlgField(nueva ? "1" : String.valueOf(tarjetaExistente.getDiaCorte()));
		JTextField tfPago = dlgField(nueva ? "20" : String.valueOf(tarjetaExistente.getDiaLimitePago()));
		JTextField tfLimite = dlgField(nueva ? "0" : String.valueOf((int) tarjetaExistente.getLimiteCredito()));

		String[] coloresLabel = { "Azul", "Verde", "Rojo", "Naranja", "Morado", "Cyan", "Rosa" };
		String[] coloresHex = { "#60A0FF", "#00CC88", "#FF5555", "#FF9933", "#AA77FF", "#00CCDD", "#FF77AA" };
		JComboBox<String> cbColor = new JComboBox<>(coloresLabel);
		if (!nueva) {
			for (int i = 0; i < coloresHex.length; i++)
				if (coloresHex[i].equals(tarjetaExistente.getColor())) {
		...
		String[] labels = { "Nombre:", "Día de corte:", "Límite de pago (día):", "Límite crédito ($, 0=sin):",
				"Color:" };
		JComponent[] fields = { tfNombre, tfCorte, tfPago, tfLimite, cbColor };
		...
		int ok = JOptionPane.showConfirmDialog(this, form, nueva ? "Nueva tarjeta" : "Editar tarjeta",
				JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
		if (ok != JOptionPane.OK_OPTION || tfNombre.getText().isBlank())
			return;

		int diaCorte = parseIntSafe(tfCorte.getText(), 1);
		int diaPago = parseIntSafe(tfPago.getText(), 20);
		double lim = parseDoubleSafe(tfLimite.getText(), 0);
		String color = coloresHex[cbColor.getSelectedIndex()];

		if (nueva) {
			tarjetas.add(new Tarjeta(tfNombre.getText().trim(), diaCorte, diaPago, lim, color));
		} else {
			tarjetaExistente.setNombre(tfNombre.getText().trim());
			tarjetaExistente.setDiaCorte(diaCorte);
			tarjetaExistente.setDiaLimitePago(diaPago);
			tarjetaExistente.setLimiteCredito(lim);
			tarjetaExistente.setColor(color);
		}
		guardar();
		actualizarCbPeriodo();
		actualizarSidebarTarjetas();
	...
	private void actualizarCbPeriodo() {
		String selActual = cbPeriodo.getSelectedItem() != null ? cbPeriodo.getSelectedItem().toString() : "Este mes";
		cbPeriodo.removeAllItems();
		cbPeriodo.addItem("Este mes");
		cbPeriodo.addItem("Esta semana");
		cbPeriodo.addItem("Todo");
		for (Tarjeta t : tarjetas)
			cbPeriodo.addItem("💳 " + t.getNombre() + " – " + t.getPeriodoLabel());
		// Restaurar selección
		for (int i = 0; i < cbPeriodo.getItemCount(); i++) {
			if (cbPeriodo.getItemAt(i).equals(selActual)) {
	...
	private JPanel buildGastosTab() {
		...
		topBar.setBorder(new CompoundBorder(new MatteBorder(0, 0, 1, 0, BORDER_LIGHT),
				BorderFactory.createEmptyBorder(10, 14, 10, 14)));
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (e.getClickCount() == 2)
					...
					eliminar.addActionListener(ev -> {
						if (JOptionPane.showConfirmDialog(FinanzasPanel.this,
								"¿Eliminar gasto \"" + g.getDescripcion() + "\"?", "Confirmar",
								JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
							gastos.remove(g);
							guardar();
							actualizarTodo();
	...
	private List<Gasto> gastosFiltrados() {
		...
		return gastos.stream().filter(g -> {
			try {
				...
				case TODO -> true;
				case TARJETA -> {
					if (tarjetaFiltro == null)
						yield true;
					LocalDate[] p = tarjetaFiltro.getPeriodoActual();
					yield !fecha.isBefore(p[0]) && !fecha.isAfter(p[1]);
				}
				};
			} catch (Exception e) {
				return false;
			}
		}).sorted(Comparator.comparing(Gasto::getFecha).reversed()).collect(Collectors.toList());
	}
	...
	private void actualizarSidebar() {
		...
		for (Categoria cat : categorias) {
			double gastoCat = filtrados.stream().filter(g -> g.getCategoriaId().equals(cat.getId()))
	...
	private JPanel buildCatCard(Categoria cat, double gastado, double totalGeneral) {
		...
		// Barra de progreso
		double pct = cat.getPresupuesto() > 0 ? gastado / cat.getPresupuesto()
				: (totalGeneral > 0 ? gastado / totalGeneral : 0);
		final double pctFinal = Math.min(pct, 1.0);

		Color barColor = cat.getPresupuesto() > 0 ? (pct >= 1.0 ? COLOR_OVER : pct >= 0.8 ? COLOR_WARN : COLOR_OK)
				: catColor;
	...
	private void actualizarTabla() {
		...
		for (Gasto g : gastosFiltrados()) {
			String nombreCat = categorias.stream().filter(c -> c.getId().equals(g.getCategoriaId()))
					.map(Categoria::getNombre).findFirst().orElse("Sin categoría");
			String fechaFmt = g.getFecha();
			try {
				fechaFmt = LocalDate.parse(g.getFecha(), FMT_ISO).format(FMT_FECHA);
			} catch (Exception ignored) {
			}
			tableModel.addRow(
					new Object[] { fechaFmt, g.getDescripcion(), nombreCat, "$" + FMT_DINERO.format(g.getMonto()) });
		}
	}
	...
	private void actualizarResumen() {
		...
		// Ingreso y disponible — siempre el ingreso completo menos lo gastado en el
		// período
		...
		List<Categoria> ordenadas = categorias.stream()
				.filter(c -> filtrados.stream().anyMatch(g -> g.getCategoriaId().equals(c.getId()))).sorted((a, b) -> {
					double ga = filtrados.stream().filter(g -> g.getCategoriaId().equals(a.getId()))
							.mapToDouble(Gasto::getMonto).sum();
					double gb = filtrados.stream().filter(g -> g.getCategoriaId().equals(b.getId()))
							.mapToDouble(Gasto::getMonto).sum();
					return Double.compare(gb, ga);
				}).collect(Collectors.toList());

		double maxCat = ordenadas.isEmpty() ? 1
				: filtrados.stream().filter(g -> g.getCategoriaId().equals(ordenadas.get(0).getId()))
						.mapToDouble(Gasto::getMonto).sum();
		...
		for (Categoria cat : ordenadas) {
			...
			resumenPanel.add(buildBarRow(cat.getNombre(), gastoCat, pctTotal, pctBar, col, cat.getPresupuesto()));
	...
	private JPanel buildBarRow(String nombre, double monto, double pctTotal, double pctBar, Color color,
			double presupuesto) {
		...
		String textoMonto = "$" + FMT_DINERO.format(monto) + "  (" + String.format("%.1f", pctTotal * 100) + "%)";
		if (presupuesto > 0)
			textoMonto += "  /  $" + FMT_DINERO.format(presupuesto);
	...
	private JPanel statCard(String titulo, String valor, Color color) {
		...
		card.setBorder(new CompoundBorder(new LineBorder(BORDER_LIGHT, 1, true),
				BorderFactory.createEmptyBorder(10, 14, 10, 14)));
	...
	private void dlgGasto(Gasto gastoEditar) {
		...
		JTextField tfFecha = dlgField(nuevo ? LocalDate.now().format(FMT_ISO) : gastoEditar.getFecha());
		...
		g.gridx = 0;
		g.weightx = 0;
		form.add(dlgLabel("Descripción:"), g);
		g.gridx = 1;
		g.weightx = 1;
		form.add(tfDesc, g);
		g.gridy = 2;
		...
		g.gridx = 0;
		g.weightx = 0;
		form.add(dlgLabel("Fecha (yyyy-MM-dd):"), g);
		g.gridx = 1;
		g.weightx = 1;
		form.add(tfFecha, g);
	...
	private void dlgCategoria(Categoria catEditar) {
		...
		if (!nuevo) {
			for (int i = 0; i < COLORES_PRESET.length; i++)
				if (COLORES_PRESET[i].equals(catEditar.getColor()))
					cbColor.setSelectedIndex(i);
		}
		...
		g.gridx = 0;
		g.weightx = 0;
		form.add(dlgLabel("Límite ($, 0=sin):"), g);
		g.gridx = 1;
		g.weightx = 1;
		form.add(tfPres, g);
		...
		btnOk.addActionListener(e -> {
			...
			if (nombre.isBlank()) {
				JOptionPane.showMessageDialog(dlg, "Escribe un nombre.");
				return;
			}
			double pres = 0;
			try {
				pres = Double.parseDouble(tfPres.getText().trim());
			} catch (Exception ignored) {
			}
	...
	private JButton accentBtn(String txt) {
		...
		b.addMouseListener(new MouseAdapter() {
			...
			public void mouseEntered(MouseEvent e) {
				b.setBackground(new Color(45, 95, 190));
			}
	...
	private void cargarDatos() {
		...
		tarjetas.addAll(FinanzasManager.cargarTarjetas(raw));
		if (categorias.isEmpty())
			cargarDefaults();
	}

	private void cargarDefaults() {
		String[][] defaults = { { "Comida", "#FF6B6B" }, { "Transporte", "#FF9F43" }, { "Entretenimiento", "#FECA57" },
				{ "Salud", "#1DD1A1" }, { "Servicios", "#48DBFB" }, { "Otros", "#A29BFE" }, };
		for (String[] d : defaults)
			categorias.add(new Categoria(d[0], d[1], 0));
	}

	private void guardar() {
		FinanzasManager.guardar(ingresoMensual, categorias, gastos, tarjetas);
	}
}