...
public class MainFrame extends JFrame {
	...
	public MainFrame() {
		...
		// ── Ícono de la ventana y barra de tareas ─────────────
		try {
			java.net.URL iconUrl = getClass().getResource("/32officeicons-15_89722.png");
			if (iconUrl != null)
		...
		btnWrapper.add(buildWowButton2());
	...
	private JButton buildWowButton() {
		...
		if (exe.exists()) {
			try {
				Icon icon = javax.swing.filechooser.FileSystemView.getFileSystemView().getSystemIcon(exe);
				Image img = ((ImageIcon) icon).getImage().getScaledInstance(32, 32, Image.SCALE_SMOOTH);
		...
		if (btn.getIcon() == null) {
			...
			btn.setBorder(new CompoundBorder(new LineBorder(new Color(180, 50, 50), 1, true),
		...
		btn.addActionListener(e -> {
			try {
				...
				JOptionPane.showMessageDialog(this, "No se pudo abrir WoW:\n" + ex.getMessage(), "Error",
	...
	private JButton buildWowButton2() {
		JButton btn = new JButton();
		btn.setToolTipText("Abrir Xenia Manager");
		btn.setFocusPainted(false);
		btn.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
		btn.setBorderPainted(false);
		btn.setContentAreaFilled(false);
		...
		java.io.File exe = new java.io.File("C:\\Users\\win11\\Downloads\\xbox\\XeniaManager.exe");
		if (exe.exists()) {
			try {
				Icon icon = javax.swing.filechooser.FileSystemView.getFileSystemView().getSystemIcon(exe);
				Image img = ((ImageIcon) icon).getImage().getScaledInstance(32, 32, Image.SCALE_SMOOTH);
		...
		btn.addActionListener(e -> {
			try {
				new ProcessBuilder("C:\\Users\\win11\\Downloads\\xbox\\XeniaManager.exe").start();
			} catch (IOException ex) {
				JOptionPane.showMessageDialog(this, "No se pudo abrir WoW:\n" + ex.getMessage(), "Error",
						JOptionPane.ERROR_MESSAGE);
			}
	...
	private JButton buildUrlButton(String label, String url) {
		...
		btn.setFocusPainted(false);
		btn.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
		btn.setBorder(new CompoundBorder(new LineBorder(new Color(45, 95, 190), 1),
